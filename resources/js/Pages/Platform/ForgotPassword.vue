...
const submit = () => form.post('/platform/forgot-password', { onFinish: () => form.reset('email') });
</script>

<template>
    <Head title="Lupa Kata Sandi — Platform" />
        ...
        <div class="w-full max-w-sm">
            ...
            <div class="mb-7">
                ...
                <p class="text-sm text-muted-foreground mt-1.5 leading-relaxed">
                    Masukkan email akun platform Anda. Kami kirimkan tautan untuk mengatur ulang kata sandi.
                </p>
            </div>

            <div v-if="flash" class="mb-5 rounded-lg border border-border bg-accent/40 px-3.5 py-3 text-sm text-foreground">
                {{ flash }}
            </div>
