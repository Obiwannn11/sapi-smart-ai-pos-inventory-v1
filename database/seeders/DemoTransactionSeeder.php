...
class DemoTransactionSeeder extends Seeder
{
     ...
     *
     * Aman dijalankan berkali-kali: hanya hari yang belum punya transaksi sama
     * sekali yang disemai, jadi menjalankannya ulang menjelang demo menambal
     * jarak sejak seed terakhir tanpa menggandakan omzet — lihat
     * `FillsMissingSalesDays`.
     *
    ...
    public function run(): void
    {
        ...
        for ($daysAgo = self::WINDOW_DAYS - 1; $daysAgo >= 0; $daysAgo--) {
            ...
            // Nomor urut kode direset tiap hari. Kode transaksi unik per
            // (tenant, code) dan tanggalnya sudah ikut di dalam kodenya, jadi
            // penomoran per hari tak pernah bertabrakan dengan hari lain —
            // sesuatu yang tidak dijamin oleh penghitung yang berjalan lintas
            // hari, karena rentang yang disemai berbeda tiap kali dijalankan.
            $txCounter = 1;
        ...
        $this->command->info("✅ DemoTransactionSeeder selesai: {$totalTransactions} transaksi pada {$hariDiisi} hari kosong.");
        $this->command->info("   {$hariDilewati} hari dilewati karena sudah punya transaksi (jendela {$start->toDateString()} s.d. {$end->toDateString()}).");
    }
