...
- **Satu angka ikut diperbaiki, ditemukan saat melihat layarnya.** Contoh di kartu itu mencetak "Rp 11000". Berkasnya sudah punya `rupiah()` dan kartu tepat di bawahnya memakainya; hanya angka ini yang lolos, dan ia berdiri persis di sebelah "Rp 10.000" yang ditulis dengan titik.
...
### [DECISION] Manajemen Stok Berhenti Jadi Accordion — Satu Baris per Varian, Disaring dan Dipaginasi di Server
- **Tanggal:** 2026-09-04
- **Fase Terkait:** Di Luar Fase — revisi tampilan, diminta pemilik.
- **Dampak:** Halaman `Owner/Stock/Index` dan `StockController::index`. Restock, adjustment, riwayat per varian, dan halaman mutasi tidak disentuh.
- **Breaking Change:** Tidak untuk pengguna; rute, request, dan nama field tetap. Untuk kode: prop `products` pada halaman itu hilang, diganti `variants` (paginator yang sudah rata), `summary`, `filters`, dan `categories`. Satu baris dataset di `tests/Feature/DeferredPageDataTest.php` ikut diperbarui; 123 test `tests/Feature/Owner` hijau.
- **Deskripsi:** Pemilik meminta bentuk lain dengan alasan yang tepat — *"bayangkan kalau jenis barang dan stok banyak, saya rasa tampilan dengan accordion tidak efektif"*. Accordion per produk yang seluruhnya dibuka saat halaman dimuat salah di tiga tempat sekaligus, dan hanya satu di antaranya soal tampilan: seluruh katalog beserta variannya dikirim dalam satu prop, seluruhnya dirender sekaligus, dan varian yang perlu ditindak tetap harus dicari dengan mata di antara ratusan baris yang baik-baik saja. Referensi bentuknya diambil dari pola *data table* shadcn (toolbar + faceted filter + paginasi); implementasinya memakai komponen dan token yang sudah ada di proyek ini — tidak ada dependensi baru.
- **Kartu status adalah penyaring, bukan papan angka.** Enam kartu di kepala halaman — Semua, Habis, Kritis, Dekat Expired, Kedaluwarsa, Aman — menjawab pertanyaan yang membawa pemilik ke halaman ini ("apa yang habis?") tanpa satu gulir pun, dan menekannya menyaring tabel di bawahnya. Angkanya dihitung atas **pencarian dan kategori yang sedang berlaku**, bukan atas seluruh katalog: kartu yang melompat balik ke angka global sesudah pemilik mengetik nama produk menyaring ke tempat yang tidak sedang dilihatnya.
- **Embernya sengaja boleh bertindih.** Varian yang habis DAN kedaluwarsa terhitung di dua kartu. Tiap kartu menjawab "berapa baris yang muncul kalau saya menekan ini", bukan membagi katalognya jadi potongan yang saling lepas — dan pembagian yang saling lepas justru menyembunyikan varian yang bermasalah karena dua alasan di kartu mana pun ia diletakkan.
- **Urutan bawaannya urgensi, bukan abjad.** `CASE` stok (habis → kritis → sisanya), lalu kedaluwarsa terdekat, lalu nama produk. Halaman ini dibuka untuk mengurus stok yang bermasalah, dan pada katalog besar baris yang bermasalah tidak akan pernah ditemukan kalau ia jatuh di halaman 14. Empat kolom tetap bisa diurutkan sendiri lewat kepala tabelnya.
- **Varian tanpa tanggal kedaluwarsa selalu di bawah, ke arah mana pun urutannya.** "Tidak punya tanggal" bukan tanggal paling awal, dan `ORDER BY expiry_date ASC` yang polos akan menaruh seluruh katalog non-perishable di atas satu-satunya barang yang sudah basi.
- **`sort`, `dir`, dan `per_page` divalidasi terhadap daftar tetap.** Ketiganya ikut menyusun SQL, dan `per_page` yang bebas diisi adalah undangan untuk meminta sejuta baris sekaligus. Nilai di luar daftar dijatuhkan ke bawaannya, bukan diteruskan.
- **Ringkasan dan tabelnya ditunda dalam SATU grup ([BL-037]).** Dipisah ke dua grup, keduanya berjalan paralel lalu tiba pada saat berbeda — angka kartunya sempat menyebut kumpulan baris yang belum terlihat. Toolbar-nya tetap eager supaya penyaringnya bisa dipakai sambil menunggu.
- **Jumlah baris per halaman jadi lima tombol angka tetap (10, 20, 25, 30, 50), diminta pemilik**, bukan dropdown penyaring — dan letaknya di kaki tabel bersama "Menampilkan 1–25 dari …". Kakinya muncul selama ada baris, bukan hanya saat halamannya lebih dari satu: kalau ia ikut hilang pada halaman tunggal, 50 baris tidak akan pernah bisa dikecilkan lagi.
- **Ambang "kritis" (≤ 5) dan "mendekati kedaluwarsa" (≤ 7 hari) punya kembaran yang harus bergerak bersama.** `StockController::LOW_STOCK_THRESHOLD` menyaring dan mengurutkan barisnya; `BadgeHelperService` memakai angka yang sama untuk lencana beranda; dan konstanta di halaman Vue-nya hanya mewarnai lencana baris yang sudah dipilih server. Batas kedaluwarsanya dibandingkan per **hari toko** di kedua sisi ([BL-082]) — `today()` di PHP sudah berada di zona bisnisnya, dan sisi peramban tetap lewat `parseDateOnly(businessToday())`.
- **Layar sempit dapat daftar kartu, bukan tabel yang digulir mendatar.** Tabel lima kolom di tablet berarti menggeser layar untuk mencapai tombol aksinya, dan aplikasi ini dipakai di tablet.
- **File Terdampak:**
  - `app/Http/Controllers/Owner/StockController.php` — `index()` ditulis ulang di atas `ProductVariant`; batas tenantnya tetap lewat `whereHas('product')` karena `ProductVariant` tidak punya `tenant_id`. Enam metode privat baru: `resolveFilters`, `baseQuery`, `applyStatus`, `applySort`, `paginateVariants`, `summarize`.
  - `resources/js/Pages/Owner/Stock/Index.vue` — accordion, `expandedIds`, dan penyaringan sisi klien dibuang; kartu status, toolbar, tabel berkepala menempel, daftar kartu untuk layar sempit, dan kaki paginasi menggantikannya. Modal restock/adjustment tidak berubah isinya.
  - `tests/Feature/Owner/StockTest.php` — 10 test baru: bentuk baris, pencarian (nama produk/varian/SKU), penyaring stok, penyaring kedaluwarsa, ringkasan, kategori, batas tenant, penolakan nilai penyaring asing, paginasi, dan urutan bawaan.
  - `tests/Feature/DeferredPageDataTest.php` — nama prop tertundanya `products` → `variants`.
...
- **Lencana jenis saran ikut masuk sistem desain — di kedua layar sekaligus.** Warnanya dulu diambil mentah dari palet Tailwind (sky, violet, amber, emerald), jadi tidak ikut tema apa pun dan tidak bisa diubah dari satu tempat. Yang lebih menentukan: keempat warna itu dipilih supaya BERBEDA, bukan supaya BERARTI — kasir harus menghafal kunci warnanya. Sekarang nadanya mengikuti seberapa besar tuntutannya: `manual` → **primary** (permintaan pemilik, yang wajib diucapkan), `pressed_stock` → **warning** (barang tertekan, ada waktunya), `upsize` → **success** (tambahan omzet nyata), `attach` → **muted** (add-on, ajakan paling ringan dan paling sering). `attach` sengaja paling tenang meski paling sering muncul: strip kasir cuma memuat tiga baris sekaligus, jadi nada tenang tidak menyembunyikannya — ia hanya berhenti berteriak sekeras permintaan pemilik.
- **Tombol "Diterima" jadi satu warna untuk keempat jenis.** Ia AKSI yang sama persis di mana pun ia berdiri — "pelanggan menerima" — dan mewarnainya per kategori membuat kasir mengira empat tombol itu berbeda akibatnya. Kategorinya sudah dibawa lencana dan bingkai kartunya. Perubahannya menyentuh `UpsellStrip.vue` **bersama** tab pratinjau, karena tab itu mengaku cermin layar kasir; mengubah satu sisi saja memecahkan cerminnya.
