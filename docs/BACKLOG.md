...
- **Status:** Open — **butuh keputusan pemilik soal BENTUKNYA sebelum ada kode.** Butir 1 di bawah tidak menunggu keputusan apa pun dan bisa dikerjakan kapan saja
- **Prioritas:** Low sebagai fitur (belum ada yang memintanya sejak 2026-08-19) / **Medium untuk butir 1**, karena di situ ada selisih harga yang benar-benar terlihat kasir hari ini
- **Area Terdampak:**
  - `app/Services/Upsell/Strategies/PressedStockStrategy.php:24-26` — docblock masih berbunyi *"Versi berdiskonnya menunggu [BL-018], karena hari ini sistem belum punya tempat sah untuk mencatat harga di bawah harga katalog"*. `[BL-018]` **selesai 2026-08-19**; tempat sah itu sudah ada
  - `resources/js/Pages/Cashier/POS.vue:548-549` — komentar yang sama, kalimat yang sama, entri yang sama
  - `app/Services/Upsell/Strategies/PressedStockStrategy.php:63,67` — `extraAmount` dan `suggestedVariantPrice` diambil dari `$variant->price`, harga KATALOG
  - `app/Services/Upsell/Strategies/UpsizeVariantStrategy.php:54,70` — sama, dari `$step->price`
  - `resources/js/Pages/Cashier/POS.vue:541,550` — baris keranjang menerima `suggestion.suggested_variant_price` apa adanya
...
- **Usulan Perbaikan (urut; butir 1 berdiri sendiri sepenuhnya):**
  1. **Tutup sisa `[BL-018]` lebih dulu — murah, dan tidak menunggu keputusan apa pun.** Strategi upsell memakai harga efektif (`DiscountService::priceFor()`) alih-alih `$variant->price`, dan dua komentar yang menunggu `[BL-018]` diperbarui. Perhatikan `extraAmount` ikut berubah artinya untuk `upsize`: selisihnya harus dihitung antara dua harga efektif, bukan dua harga katalog. Ini bukan bundling; ia menutup lubang yang sudah ada hari ini.
