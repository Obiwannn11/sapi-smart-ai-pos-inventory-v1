...
- **Area Terdampak:**
  ...
  - `app/Http/Controllers/Owner/ReportController.php:189` — `monthly()`, tidak menyebut potongan sama sekali
  - `app/Http/Controllers/Owner/ReportController.php:240` — `monthlyExport()`, empat blok CSV (ringkasan, harian, metode pembayaran, produk terlaris) dan tidak satu pun menyebut potongan
  - `app/Http/Controllers/Owner/ReportController.php:673` — `topProducts()`, memakai `SUM(transaction_items.subtotal)` yang **sudah bersih potongan**; tidak ada kolom harga normalnya
  - `app/Services/ProfitService.php:141` — laba per produk juga dari `subtotal`, jadi potongan menekan margin tanpa pernah tampil sebagai barisnya sendiri
  - `app/Services/TransactionService.php:818` — satu-satunya penulis `discount_rule_id`; penyisiran `discount_rule_id` di `app/` dan `resources/js/` tidak menghasilkan **satu pun pembaca**
  - `resources/js/Pages/Owner/Reports/Monthly.vue` — tidak ada kartu potongan
  - `resources/js/Components/TopProductsTable.vue` — empat kolom: #, Produk, Qty, Omzet
- **Deskripsi:**
  Tiga pertanyaan pemilik, dan hari ini tidak satu pun bisa dijawab dari layar:
  **G1 — "berapa total uang dipakai untuk diskon bulan ini".** Kartu "Potongan Harga" di rekap harian menjawabnya untuk SATU hari (total dipotong, jumlah baris, dan yang di bawah lantai untung berikut daftar barisnya). Rekap bulanan dan CSV-nya tidak menyebut potongan sama sekali, jadi angka sebulan hanya bisa didapat dengan membuka 30 halaman harian dan menjumlahkannya dengan tangan.
  **G2 — "diskon mana yang banyak dipakai".** `transaction_items.discount_rule_id` ditulis setiap kali aturan memotong harga, lengkap dengan alasan yang disalin ke barisnya supaya laporan bulan lalu tetap bisa menjelaskan dirinya. Kolom itu **tidak pernah dibaca**. Tidak ada satu tempat pun yang bisa menjawab berapa kali sebuah aturan terpakai, berapa rupiah yang dikorbankannya, dan berapa unit yang diselamatkannya — padahal itu satu-satunya cara owner tahu aturan mana yang layak diperpanjang.
  **G3 — "penjualan produk murni" vs setelah potongan.** Produk terlaris dan laba per produk keduanya bertumpu pada `subtotal`, yang sudah bersih potongan. Harga normalnya ada (`original_unit_price`, dan `discount_amount` per unit), tapi tidak ada yang menjumlahkannya, sehingga dua angka yang pemilik ingin lihat BERDAMPINGAN — harga normal dan yang benar-benar tertagih — tidak pernah muncul di satu tabel.
  **Bukan bagian entri ini:** potongan **paket**, yang belum punya wujud apa pun dan tetap di `[BL-103]` (butir 6 di sana sudah mensyaratkan laporan memisahkan potongan paket dari diskon per barang).
  **Kenapa baru terlihat sekarang:** `[BL-018]` menutup dirinya dengan laporan HARIAN, dan pada hari itu itulah yang dibutuhkan — potongan `near_expiry` adalah keputusan harian, dan owner memeriksanya di hari yang sama. Yang berubah adalah pertanyaannya: sesudah beberapa bulan berjalan, yang ditanyakan bukan lagi "apa yang dipotong hari ini" melainkan "berapa yang sudah kami korbankan, dan apakah itu berhasil".
- **Usulan Perbaikan:** (urutan dari pemilik 2026-09-16, termurah dulu)
  1. ~~**Blok potongan di rekap bulanan dan CSV-nya.**~~ **SELESAI 2026-09-16** — lihat `[ADDITION] Potongan Harga Terbaca Sebulan, dan Produk Terlaris Memisahkan Harga Normal dari yang Tertagih (BL-116 Butir 1 dan 2)` di `docs/CHANGELOG.md`. **Sisa kecil yang lahir pada hari yang sama:** unduhan **Excel** rekap bulanan (dikerjakan sesi lain, `[ADDITION] Rekap Bulanan Punya Unduhan Excel`) sudah memuat seluruh blok potongan ini sebagai lembar `Potongan Harga` — ia membaca `discountSummaryFor()` yang sama — tapi lembar `Produk Terlaris`-nya **belum** punya dua kolom butir 2 (Harga Normal dan Potongan) yang sudah ada di CSV. Selama itu belum ditutup, dua unduhan dari satu layar menjawab pertanyaan "per produk" dengan kedalaman yang berbeda. Kolomnya bersyarat (hanya untuk periode yang punya potongan), jadi tata letak A–E lembar itu perlu jadi A–G saat menyala — bukan tambahan satu baris, dan itu sebabnya ia dicatat alih-alih diselipkan.
  2. ~~**Kolom harga normal dan potongan di tabel produk terlaris (G3).**~~ **SELESAI 2026-09-16** — entri changelog yang sama.
  3. **Performa per aturan diskon (G2).** Baca `discount_rule_id` yang sudah tersimpan: per aturan, berapa baris penjualan, berapa unit, berapa rupiah dikorbankan, dan — bila aturannya masih ada — aturan mana yang sudah berhenti berlaku. Dua hal yang harus diputuskan sebelum mulai: (a) aturan yang **sudah dihapus** tetap punya baris penjualan (`nullOnDelete`, dan itu disengaja), jadi laporannya butuh keranjang "aturan yang sudah dihapus" yang namanya diambil dari `discount_reason` yang beku di barisnya, bukan dari tabel aturan; (b) harga khusus yang diketik owner tidak punya `discount_rule_id` sama sekali dan harus tampil sebagai kelompoknya sendiri, bukan lenyap dari total.
  4. **Potongan paket menyusul sebagai kolom keempat, bukan laporan kedua** — begitu `[BL-103]` mendarat. Alasannya ada di `[BL-103]` butir 6: "berapa yang dikorbankan untuk paket" dan "berapa untuk menghabiskan stok" dua pertanyaan berbeda, tapi keduanya dibaca di layar yang sama.
- **Yang JANGAN dilakukan:**
  1. **Jangan menghitung harga normal dari harga katalog HARI INI.** `product_variants.price` berubah, dan omzet bruto bulan lalu yang dihitung dari harga hari ini adalah angka karangan yang terlihat pasti. Sumbernya hanya dua, dan keduanya beku di barisnya: `original_unit_price`, atau `subtotal + discount_amount * qty`.
  2. **Jangan menambahkan kolom nol untuk tenant yang tidak pernah mendiskon.** Prinsip yang sama sudah berlaku untuk pajak dan biaya layanan di berkas ini: kolom nol di setiap baris bukan kejujuran, melainkan derau yang harus dibaca ulang tiap bulan oleh mayoritas yang tidak memakainya.
  3. **Jangan menjumlahkan potongan dari `transactions`.** Tabel itu tidak punya kolom potongan, dan tidak boleh diberi satu: potongan terjadi PER BARIS, dan total transaksi yang membawa ringkasannya sendiri akan berselisih dengan jumlah barisnya suatu hari — cacat yang persis sudah terjadi di `[BL-113]`.
- **Ketergantungan, dan ia sudah tertutup:** angka di entri ini pernah **kurang catat sebesar potongan yang terjadi saat offline**, karena jalur sinkron tidak menulis `discount_amount`. `[BL-115]` menutupnya pada hari yang sama (2026-09-16), jadi potongan offline kini ikut terhitung. Yang tetap di luar hanyalah baris offline yang **tersinkron sebelum** entri itu — `[BL-115]` melarang mengisinya ulang dari harga katalog hari ini, dan larangan itu berlaku di sini juga.
...
- **Status:** Open untuk butir 2 — ~~butuh keputusan pemilik soal BENTUK bundelnya sebelum ada kode~~ **bentuknya diputuskan pemilik 2026-09-15** (lihat blok "Keputusan pemilik 2026-09-15" di akhir entri). Kodenya belum ada.
...
- **Prioritas:** Low sebagai fitur — bentuknya diputuskan 2026-09-15/16, dan pemilik sudah menetapkan urutan pengerjaannya (struk lebih dulu, lihat Usulan Perbaikan butir 2). Butir 1 yang berprioritas Medium sudah mendarat
...
- **Usulan Perbaikan (urut; butir 1 berdiri sendiri sepenuhnya):**
  ...
  2. **Baru definisikan bundelnya**, sesudah keputusan di bawah turun. Keputusannya turun 2026-09-15 dan 2026-09-16. **Urutan di dalam butir ini ditetapkan pemilik 2026-09-16:**
     - ~~**2a. Struk lebih dulu**~~ **SELESAI 2026-09-16** — lihat `[ADDITION] Struk Menampilkan Potongannya: Harga Normal, Baris Diskon, dan "Anda Hemat" (BL-103 Butir 2a)` di `docs/CHANGELOG.md`. Baris potongan tercetak di struk layar (`ReceiptModal.vue`), struk printer (`escpos.js`), dan data struk API mobile, lewat satu helper `resources/js/support/discount.js`. Alasan pemilik: transparan, supaya pelanggan tidak kaget. **Paket menumpang helper yang sama begitu ia lahir:** `itemDiscount()` kini selalu berlabel "Diskon"; paket cukup memberi label baris yang lain ("Paket Nasi Goreng + Teh Manis"), bukan jalur cetak kedua.
     - ~~**2b. `[BL-115]` — jejak potongan penjualan offline**~~ **SELESAI 2026-09-16** (dipisah pemilik pada hari yang sama). Cacat yang sudah ada hari ini untuk diskon per barang (lihat "Kelemahan jalur offline" K1 dan K4), dan paket akan mewarisinya kalau dikerjakan di atasnya.
     - **2c. Paketnya sendiri**, termasuk penanganan offline P1–P6.
- **Yang perlu diputuskan pemilik sebelum butir 2 — empat pertanyaan yang menentukan skemanya, bukan tampilannya** (seluruhnya terjawab 2026-09-15, ditambah satu soal jumlah pasangan; lihat blok keputusan di akhir entri):
...
- **Yang JANGAN dilakukan saat mengerjakannya kelak:**
  ...
  2. **Jangan menegakkan lantai margin hanya di sisi harga.** `[BL-018]` menutup entrinya dengan syarat eksplisit: lantai **wajib** ikut ditegakkan di sisi SARAN. Mesin tidak pernah boleh menyarankan harga yang manusia saja boleh menembusnya. *(Satu pengecualian yang diputuskan pemilik 2026-09-16: potongan paket yang diketik owner sendiri dan dikonfirmasi lewat peringatan lantai — lihat keputusan 2. Pengecualian itu tidak berlaku untuk strategi mesin mana pun.)*
...
- **Keputusan pemilik 2026-09-15 — keempat pertanyaan di atas terjawab, ditambah satu soal jumlah pasangan yang muncul saat membahasnya:**
  1. **Bundel adalah ATURAN, bukan entitas.** Tidak ada produk "Paket" di katalog. Bentuknya "kalau beli X, Y dapat potongan": barang pemicu masuk keranjang seperti biasa, barang pasangannya ditawarkan lewat strip dan mendapat potongan saat ditambahkan. **Stok berkurang dari masing-masing barang** lewat jalur penjualan yang sudah ada, jadi tidak perlu resep komponen. Kerangkanya sudah ada di `upsell_rules` (`trigger_variant_id` → `suggested_variant_id`); yang belum ada hanya potongannya.
  2. **Owner menulis sendiri potongan dan catatannya, dan transaksinya tercatat sebagai paket.** Catatannya wajib, sama seperti `discount_rules.reason`, dan tampil di strip kasir serta tercetak di struk sebagai paket, misalnya `Paket Nasi Goreng + Teh Manis −Rp 3.000`, bukan sekadar harga Teh Manis yang tiba-tiba lebih kecil. **Lantai margin dihitung per barang yang dipotong.** ~~Aturan bundel tidak bisa menembus lantai~~ — **diubah pemilik 2026-09-16: owner BOLEH memasang potongan paket di bawah lantai, dengan peringatan saat angka yang diketik berada di bawahnya.** Peringatannya muncul di formulir sebelum disimpan, menyebut lantainya dan seberapa jauh angka itu di bawahnya; menyimpan berarti owner mengonfirmasinya dengan sadar. Karena itu:
     - Persetujuannya milik aturan, bukan milik transaksi. Aturan bundel menyimpan siapa yang menyetujui harga di bawah lantai dan kapan, lalu tiap baris penjualannya membekukan persetujuan itu ke `transaction_items.below_floor_approved_by` dan `margin_floor_at_sale`, sehingga penjualannya ikut terhitung di kartu "Di bawah lantai untung" pada laporan harian (`resources/js/Pages/Owner/Reports/Daily.vue`).
     - Kasir tetap tidak memutuskan apa pun. Ia menjual harga yang sudah disetujui owner; wewenang menembus lantai di kasir (`[BL-018]`) tidak berubah.
     - Ini **pengecualian yang disengaja** terhadap larangan "JANGAN" butir 2 di atas: harganya bukan hasil rumus mesin, melainkan diketik owner dan dikonfirmasi lewat peringatan. Strategi mesin (`pressed_stock`, `upsize`, `attach`) tetap tidak boleh menyarankan harga di bawah lantai.
  3. **Muncul di strip upsell, dengan tampilan yang berbeda dari saran biasa** (warna, bentuk kartu, label). Karena ia jenis saran baru, larangan butir 3 di atas berlaku: saklar per-tenant sejak hari pertama, dan `upsell_events` mencatatnya dengan `type` sendiri supaya konversi paket bisa dipisahkan dari saran manual biasa.
  4. **Berpasangan 1:1, dan potongan hanya berlaku untuk pasangan yang lengkap.** Nasi Goreng ×1 + Teh Manis ×3 berarti satu Teh Manis berpotongan, dua sisanya harga biasa. Kalau stok salah satu barang tidak cukup untuk menggenapkan pasangan, **layar kasir menampilkan peringatan yang menyebut sebabnya** (misalnya "Stok Teh Manis tinggal 1 — potongan paket hanya untuk 1 pasang"), bukan diam-diam menagih harga penuh. Kartu paket tidak muncul sama sekali bila salah satu barangnya berstok nol; penjaga kandidat di `ManualRuleStrategy` sudah melakukan itu untuk barang yang disarankan, dan harus diperluas ke barang pemicunya.
  5. **Bundel dan diskon tetap dua fitur terpisah, dan tidak pernah bertumpuk pada baris yang sama.** Alasan pemilik: sasarannya berbeda. Diskon per barang untuk orang yang memang hanya mau barang itu ("mau ngopi saja"); bundel untuk orang yang jadi menambah barang karena ada potongannya ("ngopi, lalu mumpung paket, ambil makanan juga"). Aturannya per baris:
     - Baris pemicu dan baris pasangan masing-masing boleh membawa potongannya sendiri. Itu bukan tumpukan, karena lantainya dijaga per baris.
     - Kalau baris pasangan memenuhi syarat keduanya, **yang berlaku potongan yang lebih besar, dan hanya satu yang tercatat.** Pelanggan yang menambah barang tidak boleh membayar lebih mahal untuk barang yang sama daripada kalau ia membelinya sendirian. Dengan aturan "bundel selalu menang", Teh Manis yang sedang diskon kedaluwarsa 30% akan NAIK harganya begitu Nasi Goreng ditambahkan, dan itu persis kejadian "kaget" yang ingin dihindari.
     - Kalau diskon per barangnya sudah lebih besar **atau sama besar**, strip tidak menampilkan kartu paket untuk barang itu, dan yang tercatat diskon per barangnya. Kartunya tidak menawarkan apa-apa, dan strip hanya boleh mengutip harga yang benar-benar ditagih (butir 1).
     - **Opsi yang ditolak, supaya tidak dibuka ulang:** *bundel selalu menang* (harga naik saat pelanggan menambah barang); *diskon selalu menang* (paket yang lebih murah diam-diam tidak berlaku); *melarang bundel memuat barang berdiskon saat aturan dibuat* (diskon `near_expiry` lahir otomatis sesudahnya, jadi larangan itu tidak menjaga apa pun); *kasir memilih* (antrean melambat, dan dua kasir bisa menagih pelanggan yang sama dengan harga berbeda).
- **Yang harus ikut dibangun supaya potongan paket tidak pernah terbaca sebagai uang hilang** (turunan keputusan di atas, bukan keputusan tersendiri):
  1. **Baris yang dipotong menunjuk aturan bundelnya DAN baris pemicunya.** `transaction_items.discount_rule_id` hanya bisa menunjuk `discount_rules` per varian, jadi bundel butuh kolomnya sendiri, `nullOnDelete` karena ia catatan sejarah. Tanpa itu laporan tidak bisa menjawab "potongan Rp 3.000 ini dari paket apa".
  2. **Server menghitung potongan paket sendiri, sesudah seluruh keranjang diketahui.** `resolveItemPrice()` (`app/Services/TransactionService.php:750`) hari ini menghitung per baris tanpa melihat baris lain. Kalau tidak diubah, salah satu dari dua hal terjadi: strip mengutip harga paket tapi server menagih harga penuh, atau server memercayai harga paket kiriman klien tanpa memeriksa pemicunya ada.
  3. **Potongan gugur saat pemicunya hilang**, di tiga tempat: keranjang, edit transaksi (`[EDIT-TX]`), dan sinkronisasi offline. Teh Manis yang tetap berpotongan setelah Nasi Goreng dihapus adalah "uang terpotong tanpa sebab" dalam bentuk paling murni.
  4. **Peringatan stok di keputusan 4 adalah soal jumlah pasangan di layar, bukan pengganti penjaga stok server.** `TransactionService` sudah menolak transaksi utuh saat stok kurang ("Stok … tidak cukup"), dan itu tetap berlaku.
  5. **Formulir aturan bundel memperingatkan, bukan menjepit** (keputusan 2, diubah 2026-09-16). Pratinjau lantai di `resources/js/Pages/Owner/DiscountRules/Index.vue` sudah menghitung lantai dan penanda "Tertahan lantai" sebelum simpan; formulir bundel memakai perhitungan yang sama, tapi hasilnya peringatan yang harus dikonfirmasi, bukan harga yang dinaikkan diam-diam. **Harga modal yang naik sesudah aturannya disimpan — diputuskan pemilik 2026-09-16:** aturan menyimpan lantai yang berlaku saat disetujui. Begitu lantai hari ini melewati angka itu, sehingga paket yang dulu di atas lantai kini di bawahnya tanpa pernah melewati peringatan, paket **berhenti berlaku di bawah lantai** (dijepit ke lantai hari ini) sampai owner mengonfirmasi ulang dari daftar aturannya. Daftar aturan menandai paket seperti itu terang-terangan, supaya owner tidak mengira paketnya sedang berjalan dengan harga yang ia tulis.
  6. **Laporan memisahkan potongan paket dari diskon per barang.** "Berapa yang dikorbankan untuk paket bulan ini" dan "berapa untuk menghabiskan stok" adalah dua pertanyaan dengan dua pemilik jawaban.
- **Persiapan sebelum butir 2 — diperiksa terhadap kode 2026-09-16:**
  1. ~~**Tunggu perbaikan rantai saran mendarat lebih dulu.**~~ **Sudah mendarat 2026-09-16** (`93a7a4b` rantai saran + batas per transaksi, `282ed22` saran berdiskon didahulukan dalam kelompoknya). Sisanya untuk self-order tetap terbuka sebagai `[BL-114]`. Bundel kini dikerjakan di atas `HEAD`, bukan di atas pohon kerja orang lain.
  2. **Struk hari ini tidak menampilkan potongan apa pun — dikerjakan PALING DULU (keputusan pemilik 2026-09-16, butir 2a).** `resources/js/Components/ReceiptModal.vue` dan `resources/js/services/escpos.js` tidak menyebut `discount_amount`, `original_unit_price`, atau kata diskon/potongan sama sekali; diskon `[BL-018]` pun hanya terlihat sebagai harga satuan yang lebih kecil. Keputusan 2 ("tercetak di struk sebagai paket") karena itu butuh baris potongan di struk lebih dulu, dan pekerjaan itu sekaligus menolong diskon per barang yang sudah ada. Struk offline dicetak dari data perangkat, jadi datanya harus tersedia di keranjang tanpa bertanya ke server.
  3. **Sinkronisasi offline akan menandai harga paket sebagai anomali — dan jalur offline punya kelemahan yang lebih dalam dari itu.** Lihat bagian "Kelemahan jalur offline" di bawah.
  4. **Pajak dan service charge tidak butuh keputusan baru.** `TaxCalculator` menghitung dari total baris SESUDAH harga server diselesaikan, jadi potongan paket menurunkan dasar pajak dengan cara yang sama seperti diskon per barang.
  5. **Tagihan terbuka tidak butuh jalur tersendiri.** Tidak ada jalur menambah barang ke tagihan yang sudah dibuka, jadi paket dihitung sekali saat tagihan dibuat.
  6. **API mobile dan self-order adalah kontrak publik.** `ApiUpsellController` terdokumentasi di `resources/views/public/api-docs.blade.php` dan `docs/phases-2/API-DOCS-Mobile.md`; kolom paket yang ditambahkan ke respons saran harus ikut tertulis di sana.
  7. **Jendela tanggal paket tunduk pada jebakan zona waktu.** Cast `date` sampai di Vue sebagai ISO UTC, sehingga `starts_on`/`ends_on` yang dipotong string tampil sehari lebih awal; format dengan zona waktu usaha.
- **Keputusan pemilik 2026-09-16 — enam pertanyaan tambahan, lima mengikuti usulan, satu diubah:**
  1. **Potongan berlaku karena ISI KERANJANG**, bukan karena tombol yang ditekan kasir. Nasi Goreng + Teh Manis yang masuk lewat grid katalog mendapat potongan paket yang sama dengan yang masuk lewat strip. Harga yang bergantung pada tombol mengulang persis cacat butir 1, dan server tidak bisa memverifikasi "datang dari strip" tanpa memercayai klien.
  2. **Owner menulis HARGA PAKET** ("Nasi Goreng + Teh Manis Rp 25.000"), karena itu yang dibaca pelanggan. Potongannya dihitung sistem dari harga katalog kedua barang dan dibulatkan ke kelipatan 500 seperti diskon.
  3. **DIUBAH PEMILIK: semua paket untuk satu pemicu harus terlihat pelanggan**, entah harga totalnya sama atau berbeda. Contoh pemilik: Nasi Goreng + Air Rp 25.000 dan Nasi Goreng + Es Jeruk Rp 26.000 tampil berdua, supaya pelanggan bisa memilih. Usulan lama ("satu paket saja, potongan terbesar lebih dulu", yang menyembunyikan sisanya) ditolak.
     - **Turunan, dikonfirmasi pemilik 2026-09-16:** satu porsi pemicu tetap membuka **satu** paket. Kalau pelanggan mengambil Air DAN Es Jeruk dengan satu Nasi Goreng, yang berpotongan adalah paket dengan penghematan terbesar bagi pelanggan (sejalan dengan keputusan 5), dan layar menyebut paket mana yang dipakai dan kenapa barang satunya harga normal.
     - **Turunan, dikonfirmasi pemilik 2026-09-16:** paket-paket untuk pemicu yang sama tampil sebagai **SATU kartu berisi daftar pilihannya**, bukan satu kartu per paket. Tanpa itu keputusan 3 bertabrakan dengan keputusan 4: Nasi Goreng dengan empat paket akan menghabiskan seluruh batas tiga saran sendirian, atau paket keempat disembunyikan — dan menyembunyikan justru yang ditolak pemilik.
  4. **Kartu paket ikut terhitung dalam batas `max_per_transaction` (3)**, supaya paket tidak jadi jalan memutar batas yang baru diperbaiki `93a7a4b`. Satu kartu berisi beberapa pilihan (turunan butir 3) dihitung satu.
  5. **Paket belum muncul di self-order.** Dikerjakan sesudah `[BL-114]` selesai, sama seperti keputusan pemilik untuk rantai saran.
  6. **Harga paket berlaku atas harga dasar varian saja.** Modifier/topping yang dipilih pelanggan tetap harga penuh dan ditambahkan di atas harga paket.
- **Dua akibat dari keputusan di atas yang harus dijaga formulir:**
  1. **Potongan tidak boleh melebihi harga barang pasangan.** Potongan menempel pada baris pasangan (keputusan 2026-09-15 butir 1). Paket Nasi Goreng Rp 20.000 + Kerupuk Rp 3.000 dengan harga paket Rp 18.000 berarti potongan Rp 5.000 pada Kerupuk seharga Rp 3.000 — baris bernilai minus. Formulir menolaknya, bukan server yang diam-diam menjepitnya.
  2. **"Harga paket" dihitung dari harga KATALOG pemicu.** Kalau pemicunya sedang punya diskon sendiri (keputusan 5 membolehkannya), total yang dibayar lebih kecil dari harga paket yang tertulis. Kartu dan struk menyebut total sebenarnya, bukan angka paket yang ditulis owner.
- **Kelemahan jalur offline — dicatat 2026-09-16 atas permintaan pemilik, diperiksa terhadap kode.**
  Prinsip yang sudah ada dan TETAP berlaku: penjualan offline sudah dibayar pelanggan, jadi server **tidak pernah menimpa** harga yang dibayar; yang boleh server lakukan hanyalah mencatat dengan benar dan menandai (`sync_status = needs_review`). Struk offline pun sudah tercetak dari data perangkat — catatan server yang berbeda dari struk di tangan pelanggan adalah kejutan yang ingin dihindari.
  **Kelemahan yang SUDAH ADA hari ini, untuk diskon per barang — bukan hanya risiko paket** (K1 dan K4 dipisah jadi `[BL-115]` 2026-09-16; K2 dan K3 tetap di sini):
  - **K1. Jejak potongan hilang pada setiap penjualan offline.** `processOfflineItems()` (`app/Services/TransactionService.php:1146`) hanya menulis `unit_price`; `original_unit_price`, `discount_amount`, `discount_rule_id`, `discount_reason`, `cost_price_at_sale`, dan `margin_floor_at_sale` tidak pernah diisi. Payload POS membawa `unit_price`, `override_unit_price`, dan `discount_reason` (`resources/js/Pages/Cashier/POS.vue:1114-1124`), tapi tidak membawa harga katalog maupun aturan diskon mana yang dipakai — jadi server tidak punya bahan untuk mengisi jejaknya. Laporan diskon menyaring `discount_amount > 0` (`app/Http/Controllers/Owner/ReportController.php:147`), jadi **potongan yang terjadi offline tidak terhitung di laporan mana pun** — ia terlihat sebagai penjualan biasa yang kebetulan lebih murah. Untuk paket, ini persis "uang terpotong tanpa sebab".
  - **K2. Harga dinilai pada waktu SINKRON, bukan waktu PENJUALAN.** `matchesAnyValidPrice()` → `priceFor()` → `rulesFor()` memakai `activeOn(now())`, dan kedalaman `near_expiry` dihitung dari `now()` (`app/Services/DiscountService.php:132,318`). Diskon yang sah saat penjualan tapi berakhir sebelum perangkat tersambung lagi jatuh ke `needs_review` palsu; diskon kedaluwarsa yang mendalam semalam membuat harga kemarin terbaca salah hari ini.
  - **K3. Snapshot tidak tahu kapan harganya berhenti berlaku.** `useCatalogCache` menyimpan `effective_price` dan indeks saran tanpa masa berlaku apa pun. Perangkat yang offline melewati tengah malam tetap menjual diskon yang sudah berakhir. Komentar di `resources/js/composables/useCatalogCache.js:56-59` ("indeks ini tidak pernah membawa harga diskon") **sudah usang** sejak butir 1: saran kini membawa harga berdiskon.
  - **K4. Harga khusus owner tidak punya jalur offline.** Perangkat sudah mengirim `override_unit_price` dan `discount_reason`, tapi `processOfflineItems()` tidak membaca keduanya, sehingga harga khusus yang dimasukkan owner saat offline tercatat tanpa alasan dan tanpa `below_floor_approved_by`, lalu jatuh ke `needs_review` karena harganya tidak cocok dengan aturan mana pun.
  **Edge case khusus paket, beserta usulan penanganannya:**
  - **P1. Paket berakhir, dinonaktifkan, atau harganya diubah owner saat perangkat offline.** Perangkat tetap menjual harga paket lama. → Nilai terhadap aturan yang berlaku pada `occurred_at`. Untuk itu **harga paket tidak diedit di tempat**: perubahan harga menyimpan versi baru, supaya harga yang sah kemarin masih bisa dibuktikan hari ini. Cocok dengan versi yang berlaku saat penjualan → sah, tanpa penanda.
  - **P2. Harga paket tanpa barang pemicu di transaksi yang sama** (payload rusak atau dimanipulasi, atau pemicu dihapus dari keranjang tapi harga pasangan tidak kembali). → Harga yang dibayar tetap dicatat, tapi ditandai dengan alasan spesifik "potongan paket tanpa barang pemicu", bukan penanda umum.
  - **P3. Peringatan pasangan (keputusan 2026-09-15 butir 4) memakai stok snapshot yang hanya perkiraan.** → Offline, pasangan tidak ditahan karena stok snapshot; peringatannya diberi label "stok perkiraan". Pengurangan stok tetap optimistik seperti hari ini, dan stok minus tetap ditandai (`went_negative`).
  - **P4. Harga modal naik saat perangkat offline**, sehingga paket seharusnya sudah tertahan di lantai (keputusan harga modal 2026-09-16) tapi perangkat belum tahu. → Sah karena sudah dibayar; `margin_floor_at_sale` diisi dari versi aturan yang berlaku saat penjualan, dan barisnya masuk hitungan "di bawah lantai" dengan alasan "lantai berubah sebelum perangkat tersambung".
  - **P5. "Ambil yang lebih besar" (keputusan 5) dihitung perangkat dengan diskon basi.** → Server menghitung ulang pada `occurred_at`. Kalau hasilnya berbeda, harga yang dibayar tetap dicatat dan ditandai dengan alasan spesifik.
  - **P6. Struk offline dan catatan server tidak boleh berbeda.** → Seluruh penanganan di atas hanya boleh menambah penanda dan alasan, tidak pernah mengubah angka yang sudah tercetak.
  **Usulan perbaikan (urut):**
  1. **Payload offline membawa jejak potongannya per baris:** jenis potongan (diskon / paket / harga khusus), id aturan dan versinya, baris pemicu untuk paket, dan harga katalog yang dilihat perangkat. Server **memverifikasi klaim itu, tidak memercayainya**: kolom jejak diisi dari aturan server; klaim yang tidak bisa dijelaskan aturan mana pun ditandai beserta alasannya. Menutup K1, K4, P2.
  2. **Aturan dinilai pada `occurred_at`, bukan `now()`.** `DiscountService::rulesFor()`/`priceFor()` menerima tanggal, dan paket punya riwayat versi harga. Menutup K2, P1, P4, P5.
  3. **Snapshot membawa batas berlaku harga:** `valid_until` = yang lebih awal antara akhir hari usaha saat snapshot diambil (zona waktu usaha) dan `ends_on` aturannya. Lewat batas itu, perangkat offline berhenti menerapkan diskon dan paket, lalu menjual harga katalog. Harga katalog selalu sah saat sinkron, dan pelanggan melihat harganya di layar sebelum membayar, jadi tidak ada kejutan. Harganya: pelanggan kehilangan potongan kalau perangkat offline melewati tengah malam — jarang, dan lebih baik daripada menjual potongan yang sudah berakhir. Menutup K3.
  4. **Penanda `needs_review` membawa alasan yang spesifik** (paket tanpa pemicu, versi aturan tidak dikenal, lantai berubah, hitungan "lebih besar" berbeda), supaya owner yang membukanya tahu apa yang harus diperiksa tanpa membandingkan angka sendiri.
  5. **K1 dan K4 cacat yang sudah hidup hari ini** untuk diskon per barang. **Dipisah jadi `[BL-115]` (keputusan pemilik 2026-09-16)**, dikerjakan sesudah struk dan sebelum paket (Usulan Perbaikan butir 2b). Usulan 1 di atas untuk diskon per barang dan harga khusus dikerjakan di sana; bagian paketnya (klaim baris pemicu, versi harga paket) tetap di sini.
...
- **Status:** In Progress — **bentuknya diputuskan pemilik 2026-09-15** (lihat blok "Keputusan pemilik 2026-09-15" di akhir entri). **Tahap 1 selesai 2026-09-15:** UI kelola link di halaman Integrasi (`[ADDITION] Link Data untuk AI di Halaman Integrasi: Owner Membuat, Melihat, dan Mencabut Link Berumur, Endpoint Datanya Menyusul (BL-102 Tahap 1)`). **Tahap 2 selesai 2026-09-15:** endpoint `/api/v1/connector/summary` (`[ADDITION] Link Data untuk AI Kini Membuka Data Toko: Satu Paket Teks Tanpa Parameter, dan Setiap Penolakan Terbaca Sebagai Penolakan (BL-102 Tahap 2)`). **Belum:** uji prompt dengan link sungguhan di Claude, ChatGPT, dan Gemini; layanan AI tidak menjangkau `localhost`, jadi menunggu deploy. Sampai `[BL-113]` selesai, uji itu sebaiknya memakai Squid Coffee & Eatery, karena laba data demo Kopi Nusantara dan Kopi Story terbaca minus. Butir 1 diubah pemilik pada hari yang sama sesudah `[BL-112]` ditutup: link konektor kini **token Sanctum**, bukan tabel sendiri
...
- **Keputusan pemilik 2026-09-15 — keempat pertanyaan di atas terjawab, semuanya mengikuti rekomendasi:**
  ...
  4. **Penyampaian bahaya: dialog + centang + riwayat pakai.** Tidak ada tombol "Salin URL" langsung. Membuat link membuka dialog: pilih masa berlaku, lalu centang kalimat konkret bertanggal ("Siapa pun yang memegang link ini bisa melihat penjualan dan profit toko saya sampai 15 Okt 2026"). URL tampil **sekali**, seperti token MCP. Daftar link aktif menampilkan label, tanggal mati, dan "terakhir dipakai". *(Diperbarui pemilik 2026-09-15: URL tampil di modal yang tidak tertutup lewat klik di luar atau Esc, dan Tutup/X baru berlaku setelah link benar-benar tersalin, lewat clipboard atau salin manual dari kotak prompt. Tanggal mati ditulis di kalimat di bawah tombol salin, bukan di label tombolnya.)*
