...
>
> Urutan yang disarankan, termurah dulu: `[BL-067]` (keterlihatan seat/kuota) → `[BL-066]` (penjelasan Dynamic Pricing di landing) → ~~`[BL-065]`~~ (PPN — **selesai 2026-08-29**, diarsipkan) → `[BL-068]` (cabang, paling akhir). Tiga yang paling murah — `[BL-062]` (kuota AI), `[BL-063]` (rekap bulanan), dan `[BL-064]` (grafik garis) — selesai 2026-08-13; ketiganya memang tidak menyentuh uang sama sekali.
...
- **Status:** Open — **butuh keputusan, bukan pekerjaan.** Empat pertanyaan di bawah menentukan kolom mana yang lahir; menjawabnya tanpa pengguna yang benar-benar memungut berarti membekukan tebakan ke dalam basis data.
...
### [BL-098] `ProfitService` Menghitung HPP dengan Harga Modal Hari Ini, Padahal Harga Modal Saat Jual Sudah Dibekukan
- **Ditemukan:** 2026-08-29 (saat memutuskan dasar margin `[BL-065]`)
- **Sumber:** Terlihat saat mengerjakan keputusan dasar margin; sengaja ditinggalkan di luar keputusan itu supaya satu pertanyaan tidak menyeret pertanyaan lain.
- **Status:** Open — **pertanyaan, bukan bug.** Sudah tercatat sebagai keterbatasan yang diketahui di docblock `ProfitService`; yang belum diputuskan adalah apakah layak diperbaiki.
- **Prioritas:** Low
- **Duduk perkaranya:**
  `app/Services/ProfitService.php` menghitung COGS sebagai `SUM(qty × product_variants.cost_price)` — harga modal **hari ini**. Sementara `transaction_items.cost_price_at_sale` sudah membekukan harga modal per baris sejak `[BL-018]`, justru supaya "apakah ini tetap untung" bisa dihitung ulang setelah harga modal berubah — dan harga modal pasti berubah.

  Akibatnya margin bulan lalu ikut bergerak setiap kali pemilik memperbarui harga modal, tanpa satu pun transaksi berubah. Distorsi kedua pada angka yang sama dengan yang baru diperbaiki 2026-08-29, dan pembacanya sama: konteks analisis AI dan MCP `GetProfitTool`.
- **Yang perlu diputuskan:**
  - **Apakah harga modal beku itu selalu ada.** Baris yang lahir sebelum `[BL-018]` punya `cost_price_at_sale` null, dan `COALESCE` ke harga hari ini berarti satu laporan memakai dua dasar sekaligus — mungkin lebih menyesatkan daripada konsisten salah.
  - **Apakah margin historis memang seharusnya beku.** Ada bacaan sah yang sebaliknya: pemilik yang bertanya "produk ini masih untung tidak?" sedang bertanya tentang harga modal **sekarang**, bukan tentang harga modal tiga bulan lalu. Kalau itu yang dimaui, yang salah bukan kodenya melainkan namanya — dan jawabannya menambah angka kedua, bukan mengganti yang pertama.
